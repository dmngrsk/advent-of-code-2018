...
let rec findLastCartCoordinates (grid:list<string>, carts) =
    match carts with
    | (x, y, _, _) :: [] -> x, y
    | _ -> findLastCartCoordinates (grid, fst (moveCartsInOrder grid carts))

let input = File.ReadAllLines("./input.txt")
let x, y = findLastCartCoordinates (input |> parse)
printfn "%d,%d" x y