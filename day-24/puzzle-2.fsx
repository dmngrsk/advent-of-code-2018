...
let parseLine i f boost line = 
    ...
    let wks, ims = parseWeaknessesAndImmunities (group id 3)
    {
        ...
        Weaknesses = wks; 
        Immunities = ims;
        Damage = (group int 4) + boost;
        DamageType = group id 5;
        Initiative = group int 6
    }

let parse input immuneBoost =
    let ims, ifs = 
        input 
        |> List.ofArray 
        |> List.splitAt (Array.findIndex (fun s -> s = "") input)
    let immuneSystemGroups = ims |> List.tail |> List.mapi (fun i l -> parseLine i Fraction.ImmuneSystem immuneBoost l)
    let infectionGroups = ifs |> List.skip 2 |> List.mapi (fun i l -> parseLine (i + List.length immuneSystemGroups) Fraction.Infection 0 l)
...
let rec findOutcome ugs =
    ...
    | Some _ -> ugs', List.sumBy (fun ug -> ug.Count) ugs'
    | None when ugs = ugs' -> ugs', -1
    | None -> findOutcome ugs'

let rec findOutcomeWithMinimalBoost min max input =
    let mid = max - (max - min) / 2
    let ugs = parse input mid
    match findOutcome ugs with
    | ugs', out when min = max -> out
    | ugs', out when min = (max - 1) && getWinningFraction ugs' = Some Fraction.ImmuneSystem -> out
    | _, _ when min = (max - 1) -> findOutcomeWithMinimalBoost min min input
    | ugs', _ when getWinningFraction ugs' = Some Fraction.ImmuneSystem -> findOutcomeWithMinimalBoost min mid input
    | _, _ -> findOutcomeWithMinimalBoost mid max input

let input = File.ReadAllLines("./input.txt")
printfn "%A" (findOutcomeWithMinimalBoost 0 10000 input)