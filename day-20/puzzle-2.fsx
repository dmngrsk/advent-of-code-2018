...
let countPathsOfLength l paths = paths |> List.filter (fun (_, l') -> l' >= l) |> List.length

let input = File.ReadAllText("./input.txt")
printfn "%d" (input |> Seq.map id |> List.ofSeq |> buildGraph (0, 0) |> shortestPaths (0, 0) |> countPathsOfLength 1000)